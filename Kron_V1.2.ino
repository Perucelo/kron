...
unsigned long Minutos        =    60 * Segundos;
unsigned long TempoDeEspera =     30 * Segundos;
unsigned long ReguladorEspera =   30 * Segundos;
unsigned long TempoDeReset  =     1 * Minutos;  
unsigned long TempoDeEnvio  =     30 * Segundos;
unsigned long ReguladorEnvio  =   30 * Segundos;
...
void loop() {
   ...
   if (millis() > TempoDeEnvio)
  {
     
      Internet();  
      Serial.print("Time Internet: ");
      Serial.println(TempoDeEnvio);
      TempoDeEnvio = ReguladorEnvio + millis();
  ...
  if(millis() > TempoDeEspera )
  {
     ...
     Serial.print("Time Modbus: ");
     Serial.println(TempoDeEspera); // prints time since program started
     TempoDeEspera = ReguladorEspera + millis();
...
void Internet()
{
   ...
   if (conn.connected()) 
   {
       sprintf(sentenca, INSERT_SQL,2,VBD0,IBD0,FPBD,S0BD,Q0BD,P0BD,FBD,V1BD,V2BD,V3BD,IL1BD,IL2BD,IL3BD,PABD,PBBD,PCBD,QABD,QBBD,QCBD,FPABD,FPBBD,FPCBD,EAPBD);      
